...
for t = 1 : IterMax
    ...
    for i = 1 : NumExpl
        if R2 < ST                                          % 预警值较小，没有捕食者出现
            Alpha = rand();
            Pop(Index(i)).Position = Pop(Index(i)).Best.Position * exp(-i / (Alpha * IterMax));
    ...
    % 3.3.防御者的位置更新
    randIndex = randperm(PopSize);                      	% 在种群中产生随机位置，前面dNum个是防御者
    dIndex = randIndex(1 : NumDefe);                           % 意识到危险的麻雀的位置
    for i = 1 : NumDefe
        if Pop(dIndex(i)).Fitness == Best.Fitness           % 处于种群中心的麻雀
            Pop(dIndex(i)).Position = Pop(dIndex(i)).Best.Position + (2 * rand() - 1) * abs(Pop(dIndex(i)).Best.Position - Worst.Position) / ((Pop(dIndex(i)).Best.Fitness - Worst.Fitness) + 1e-50);
        else                                                % 处于种群外围的麻雀
            Pop(dIndex(i)).Position = Best.Position + randn(1,dim) .* abs(Pop(dIndex(i)).Best.Position - Best.Position);
        end
    end
...
%% 4.输出结果与数据可视化
disp('The best solution obtained by SSA is :')
disp(num2str(GlobalBest.Position))

disp('The best optimal value of the objective funciton found by SSA is :')
