...
CoefPers = 2.05;                            % 个体学习因子
CoefGlob = 2.05;                         	% 群体学习因子
...
for t = 1 : IterMax
    ...
    MeanBest = zeros(1,dim);                              	% 平均最好位置
    for i = 1 : PopSize 
        MeanBest = MeanBest + 1 / PopSize * Pop(i).Best.Position;
    end
    
    Beta = WeightMin + (WeightMax - WeightMin) * (IterMax - t) / IterMax;       % 线性递减更新权重
    ...
    for i = 1 : NumExpl
        if R2 < ST                                          % 预警值较小，没有捕食者出现
            Alpha = rand();
            if rand() <= 0.5
                Pop(Index(i)).Position = Pop(Index(i)).Best.Position * (1 + exp(-t / (Alpha * Beta * IterMax)));
            else
                Pop(Index(i)).Position = Pop(Index(i)).Best.Position * (1 - exp(-t / (Alpha * Beta * IterMax)));
            end
    ...
    for i = NumExpl + 1 : PopSize
        ...
        if i <= PopSize / 2                                 % 这部分追随者围绕最好的探索者周围进行觅食，其间也有可能发生食物的争夺，使其自己变成探索者
            ...
            r1 = rand(1,dim);
            r2 = rand(1,dim);
            L = (CoefPers * r1 .* Pop(Index(i)).Best.Position + CoefGlob * r2 .* GlobalBest.Position) ./ (CoefPers * r1 + CoefGlob * r2);      % 吸引子
            if rand() <= 0.5
                Pop(Index(i)).Position = L + Beta .* abs(MeanBest - Pop(Index(i)).Best.Position) .* log(1 ./ rand(1,dim));
            else
                Pop(Index(i)).Position = L - Beta .* abs(MeanBest - Pop(Index(i)).Best.Position) .* log(1 ./ rand(1,dim));
            end
        end
